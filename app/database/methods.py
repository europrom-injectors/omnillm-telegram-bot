...
class ChatMethods(PostgresConnectionWithContext):
    ...
    async def update_chat_agent(self, agent: str, chat_id: int = None) -> None:
        query = "UPDATE chats SET agent = $1 WHERE id = $2"
        await self.execute(query, (agent, chat_id or (await self.get_active_chat()).id))

    async def update_chat_llm_model(self, model: str, chat_id: int = None) -> None:
        query = "UPDATE chats SET llm_model = $1 WHERE id = $2"
        await self.execute(query, (model, chat_id or (await self.get_active_chat()).id))

    async def update_chat_online_model(
        self, online_model: bool, chat_id: int = None
    ) -> None:
        query = "UPDATE chats SET online_model = $1 WHERE id = $2"
        await self.execute(
            query, (online_model, chat_id or (await self.get_active_chat()).id)
        )
